        ...
        for y in range(Const.GRID_SIZE):
            for x in range(Const.GRID_SIZE):
                ...
                if ball >= 0:
                    ...
                    ball_end = vec2(ball_origin.x + VisuConst.BALL_SIZE, ball_origin.y + VisuConst.BALL_SIZE)
                    color = VisuConst.PLAYER_COLOR[ball]
                    canvas.create_oval(ball_origin.x, ball_origin.y, ball_end.x, ball_end.y, fill=color, width=VisuConst.DRAW_WIDTH)
